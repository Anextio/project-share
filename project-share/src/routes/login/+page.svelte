<script>
  ...
  <div class="flex justify-center items-center min-h-screen p-4">
	<Card class="w-full max-w-md p-6">
	  <Tabs bind:activeTab={activeTab} data-testid="tabs">
	  ...
	  {#if activeTab === 0}
		<div class="flex flex-col items-center">
		  ...
		  <Button on:click={signInWithGoogle} variant="contained" color="secondary" class="mt-4" disabled={loading}>
			{#if loading}
			  <span class="animate-spin">&#9696;</span>
			{/if}
			Sign in with Google
	  ...
	  {:else}
		<form class="flex flex-col gap-4 mt-6" data-testid="signUpForm">
		  <Input bind:value={email} label="Email" type="email" required class="w-full" />
		  <Input bind:value={password} label="Password" type="password" required class="w-full" />
